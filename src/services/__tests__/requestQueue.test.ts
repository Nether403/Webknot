...
describe('RequestQueue', () => {
  ...
  describe('enqueue', () => {
    ...
    it('should throw error when queue is full', async () => {
      ...
      // This should fail
      await expect(queue.enqueue(slowOperation, 'user4')).rejects.toThrow('Request queue is full');
    ...
    it('should process requests concurrently up to maxConcurrent', async () => {
      ...
      // Enqueue 5 requests
      const promises = Array.from({ length: 5 }, (_, i) => queue.enqueue(operation, `user${i}`));
  ...
  describe('retry logic', () => {
    ...
    it('should not retry non-retryable errors', async () => {
      ...
      await expect(queue.enqueue(failingOperation, 'user1')).rejects.toThrow('Invalid API key');
  ...
  describe('timeout', () => {
    it('should timeout long-running requests', async () => {
      ...
      await expect(queue.enqueue(slowOperation, 'user1')).rejects.toThrow('timeout');
    });
  });
});
